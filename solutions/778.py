...
class Solution:
    def bfs(self, grid, t) -> bool:
        ...
        while len(queue) > 0:
            ...
            print(cur_x, cur_y)
            if cur_x == cur_y == size-1:
                return True
            if t < grid[cur_x][cur_y]:
                continue
            for move in moves:
                new_x = cur_x + move[0]
                new_y = cur_y + move[1]
                if new_x >= 0 and new_y >= 0 and new_x < size and new_y < size:
                    if (not visited[grid[new_x][new_y]]) and t >= grid[new_x][new_y]:
                        queue.append((new_x, new_y))
                        visited[grid[new_x][new_y]] = True
        return visited[grid[size-1][size-1]]
